        ...
        <div v-if="search_q.length < 3" class="col-12 col-md-12 border-c py-3" style="height: 88vh;">
          <div class="row">
            <div class="col col-4 col-md-4 col-sm-12">
              ...
              -->
              <div class="search">
                <select class="form-control" v-model="sort_by" @change="searchFilterSelect">
          ...
          <div class="row">
            <div class="col-12" style="margin-top: 20px">
              ...
              <div class="row">
                <div class="col-md-12">
                  <h2 class="text-white float-left">{{title}}</h2>
                  <select id="" class="form-control float-right" @change="handleChange($event.target)">
                    <option v-for="option in dropDown" :id="option.id" v-text="option.title"></option>
                  </select>
                </div>
              </div>
              ...
              <div v-if="selectedOpt == 4"
                   class="row mt-2 mt-md-4">
                <div
                  v-for="result in filterItems(this.followed)"
                  :id="'following-' + result._id"
                  v-bind:key="result.id"
                    ...
                    <div class="bgdark col-12">
                      <div class="row">
                        <div class="col-6 text-center">
                          ...
                          <h6 class="text-white">
                            <router-link class="text-white" v-if="result.user"
                                         :to="{ name: 'profile_view', params: {username: result.user.username } }">
                              <span>{{result.user.firstName}} {{result.user.lastName}}</span></router-link>
                          </h6>
                        </div>
                        <div class="col-6 text-right">
                          <div class="clearfix"></div>
                          <button v-if="checkIfAlreadyFollow(result.user)" @click="unfollowUser(result.user)"
                    ...
                    <div class="row">
                      ...
                      <div class="col-6 text-center">
                        ...
                        <a @click="declineFriendRequest(result.createdBy)"
                           class="fs13 btn btn-xs btn-warning text-white"
                           style="padding: 0 5px;font-size: 11px">Remove Friends Requests</a>
                        <div class="clearfix"></div>
              ...
              <div v-if="selectedOpt == 2"
                   class="row mt-2 mt-md-4 "
              >
                <div v-for="result in filterItems(results)"
                     v-bind:key="result.id" class="bgdark-div col-12 col-md-6">
                  <div class="bgdark col-12">
                    <div class="row">
                      <div class="col-6 text-center">
                        <i class="material-icons text-white">account_circle</i>
                      ...
                      <div class="col-6 text-center">
                        ...
                        <a @click="declineFriendRequest(result.createdBy)"
                           class="fs13 btn btn-xs btn-warning text-white"
                           style="padding: 0 5px;font-size: 11px">Remove Friend</a>
                        <div class="clearfix"></div>
                        <button v-if="checkIfAlreadyFollow(result.userData)" @click="unfollowUser(result.userData)"
                                class="fs13 btn btn-xs btn-danger" style="padding: 0 5px;font-size: 11px">Un-Follow
                        </button>
    ...
    export default {
        ...
        data() {
            return {
                ...
                selectedOpt: 1,
                dropDown: [
                    {id: 1, title: 'Friends List'},
                    {id: 2, title: 'Pending Friends Requests'},
        ...
        methods: {
            handleChange(elem) {
                let listOption = elem.options[elem.selectedIndex];
                this.title = listOption.text;
                this.selectedOpt = listOption.id;
            },

            dragStart(event) {
                let target = event.target;
                let elemData = target.id.split('-');
                let object = {friend_id: target.id, target_type: elemData[0]};
